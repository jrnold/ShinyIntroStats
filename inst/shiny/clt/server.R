...
shinyServer(function(input, output) {
    ...
    data <- reactive({
        ...
        FUN <- switch(input$distribution,
                      ...
                      beta = function() rbeta(obs, as.numeric(shape1), as.numeric(shape2)),
                      gamma = function() rgamma(obs, as.numeric(shape), as.numeric(scale)),
    ...
    output$pop_mean <- renderText({
        if (input$distribution == "normal") {
            ...
            mu <- as.numeric(shape1) / (as.numeric(shape1) + as.numeric(shape2))
        } else if (input$distribution == "gamma") {
            mu <- as.numeric(shape) * as.numeric(scale)
